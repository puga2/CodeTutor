...
const Navbar = () => {
    ...
    return (
        <nav className='bg-black z-50 text-white  fixed top-0 left-0 right-0 border-b border-white/10'>
            <div className='container mx-auto px-4 py-2 sm:px-6 lg:px-8'>
                <div className='flex items-center justify-between h-16'>
                    ...
                    <div className='hidden lg:flex  items-center space-x-4'>{
                        Object.keys(menuItems).map((key) => (
                            <div key={menuItems[key].title} className='relative'>
                                <button onClick={() => toggleDropdown(key)} className='hover:text-gray-300 px-3 py-2
                                            ...
                                            <div className='grid md:grid-cols-2 grid-cols-1 gap-12 p-4'>{
                                                    key === 'platform' ? (
                                                        menuItems[key].sections.map((section,idx)=>(
                                                            <div key={idx}>
                                                                ...
                                                                <Link key={itemIdx} to={`/${key}/${item.name.toLowerCase()}`}
                                                                    className='group flex items-start p-2 rounded-lg hover:bg-gray-50'>
                                                                    <div className='px-4'>

                                                                        <p className='text-sm font-medium text-gray-900 flex items-center'>{item.name} {item.isNew && (<span className='ml-2 text-xs font-medium text-blue-800 items-center px-2 py-0.5 inline-flex bg-blue-100 rounded-xs'>NEW</span>) }</p>
                                                    ...
                                                    :  (<div className='space-y-2'>
                                                        {menuItems[key].items.map((item,index)=>(
                                                            <Link to={`/${key}/${item.name.toLowerCase()}`}
                                                                className='group flex items-start p-2 rounded-lg hover:bg-gray-50'>
                                                                <div className='px-2'>
                    ...
                    <div className='hidden md:flex items-center space-x-4'>
                        ...
                        <Link to="/contact" className='hover:text-gray-300 hidden xl:block'>Contact Sale</Link>
                        <Link to="/get-started" className='hover:text-gray-300 bg-blue-600 px-4 py-2 hover:bg-blue-700  rounded-md '>Get Started - it's free</Link>
                    </div>
                    {/* mobile menu */}
                    <div className='md:hidden '>
                        ...
                        hover:text-gray-300 hover:bg-gray-700'>
                            {
                                !isMenuOpen ? (<MdMenu className='block size-6'/>):(<MdClose className='block size-6'/>)
                            }
                        </button>
                    </div>
                ...
                isMenuOpen && (
                    <div className='md:hidden'>
                        <div className="px-2 pt-2  pb-3 space-y-1">
                                                            ...
                                                            <div key={idx} className='py-2 '>
                                                                ...
                                                                <Link key={itemIdx} to={`/${key}/${item.name.toLowerCase()}`}
                                                                    ...
                                                                    <div className='px-4'>
                                                                        ...
                                                                        <p className='text-sm font-medium text-gray-900 flex items-center'>{item.name} {item.isNew && (<span className='ml-2 text-xs font-medium text-blue-800 items-center px-2 py-0.5 inline-flex bg-blue-100'>NEW</span>) }</p>
                                                                        {/* <p className='text-sm text-gray-500'>{item.desc}</p> */}
                                                                    </div>
                                                                </Link>
                                                                    ))}
                            ...
                            <Link to="/enterprise" className='block px-3 py-2 hover:bg-gray-700'>Pricing</Link>
                            <Link to="/enterprise" className='block px-3 py-2 hover:bg-gray-700'>Login</Link>
                            <Link to="/enterprise" className='block px-3 py-2 hover:bg-gray-700'>Contact Sales</Link>
                            <Link to="/enterprise" className='block px-3 py-2 rounded-md bg-blue-600 hover:bg-blue-700'>Get Started - it's free </Link>
